...
FAST_datalist <- function(Y, N, K, Q, Domain, W, basis_type = "Splinet", 
                          alpha = 0.1, scale = T){
  ...
  # Format for FAST
  fast_list = list(N = N, M = M_val, Q = Q, K = K, Y = scaled_Y, 
                   B = B, P_alpha = P_alpha, sd_Y = sd_Y, mu_Y = mu_Y)
  return(fast_list)
}
...
FAST_extract <- function(mod_fit, B, Domain, DL){
